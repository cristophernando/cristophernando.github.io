(async function() {
    ...
    try {
        ...
        const response = await fetch(`https://cristophernando.github.io/checksite/${domainmodified}.json?date=1000`);
        ...
        if (hoy > expiration_date && balance > 0) {
            console.log('If you are seeing this is because this website has not paid to its developers or designers');
            body.style = "background:#f1f1f1; color:#444; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif; margin:0; padding:0;";
            body.innerHTML= `
            <div style="max-width:700px; margin:50px auto; background:#fff; border:1px solid #ccd0d4; box-shadow:0 1px 1px rgba(0,0,0,0.04); padding:1em 2em;">
                ...
                </p>` + (data['show_link']?'<p style="font-size:14px; line-height:1.5; margin:0;">Si continúas teniendo problemas, intenta contactar con el soporte.</p>':'') + '</div>';
            return;
        }
